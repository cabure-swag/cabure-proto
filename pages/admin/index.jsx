...
export default function Admin() {
  ...
  function onChange(e){ setForm(f=> ({...f, [e.target.name]: e.target.name==='active'? e.target.checked : e.target.value })); }
  function onAssignChange(e){ setAssign(a=> ({...a, [e.target.name]: e.target.value })); }

  async function createBrand(){
    const payload = { name: form.name, slug: form.slug, color: form.color, active: form.active };
    const { error } = await supabase.from('brands').insert(payload);
    if(error) alert(error.message); else { setForm({ name:'', slug:'', color:'#7c3aed', active:true }); await loadBrands(); }
  }

  async function delBrand(b){
    if(!confirm('¿Eliminar esta marca?')) return;
    const { error } = await supabase.from('brands').delete().eq('id', b.id);
    if(!error) await loadBrands();
  }
  ...
  async function assignVendor(){
    ...
    if(!prof){ alert('No existe perfil con ese email (el usuario debe loguearse al menos una vez).'); return; }
    await supabase.from('brand_users').insert({ brand_id: assign.brand_id, user_id: prof.user_id });
    setAssign({ brand_id:'', email:'' });
    alert('Vinculado.');
  }

  if(role !== 'admin'){
    return <main className="container"><h1>Admin</h1><p className="muted">Necesitás rol de admin.</p></main>;
  }
    ...
    <main className="container">
      ...
      <section className="card">
        ...
        <div className="row" style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12}}>
          ...
          <div className="row" style={{display:'flex', alignItems:'center', gap:8}}>
            <input type="checkbox" name="active" checked={form.active} onChange={onChange} /><label>Activa</label>
          </div>
        </div>
      ...
      <section className="card" style={{marginTop:12}}>
        ...
        <div className="row" style={{display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:12}}>
          <div className="row">
            <label>Marca</label>
            <select className="input" name="brand_id" value={assign.brand_id} onChange={onAssignChange}>
          ...
          <div className="row">
            ...
            <input className="input" name="email" value={assign.email} onChange={onAssignChange} placeholder="vendedor@correo.com" />
          </div>
          <div style={{alignSelf:'end'}}>
            <button className="btn btn-primary" onClick={assignVendor}>Asignar</button>
          </div>
        </div>

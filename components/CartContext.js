...
export function CartProvider({children}){
  ...
  const addItem=(product,qty=1)=>setItems(prev=>{const i=prev.findIndex(x=>x.id===product.id); if(i>=0){const cp=[...prev]; cp[i]={...cp[i], qty:cp[i].qty+qty}; return cp;} return [...prev,{...product,qty}];});
  const removeItem=(id)=>setItems(prev=>prev.filter(x=>x.id!==id));
  const clearCart=()=>setItems([]);
  const total=items.reduce((s,p)=>s+p.price*p.qty,0);
  return <Ctx.Provider value={{items,addItem,removeItem,clearCart,total}}>{children}</Ctx.Provider>
}
